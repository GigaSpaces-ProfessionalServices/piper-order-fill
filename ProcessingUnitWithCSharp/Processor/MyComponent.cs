...
namespace GigaSpaces.Examples.ProcessingUnit.Processor
{
    ...
    class MyComponent
    {
        ...
        public void postBackupEvent(ISpaceProxy proxy, SpaceMode spaceMode)
        {
            ...
            Logger.Write(">>>>> post backup -> postBackupEvent " + proxy.Name + ", spaceMode: " + spaceMode.ToString()
                + " isStartedAsPrimary: " + isStartedAsPrimary.ToString());

            Logger.Write("clusterInfo : " + clusterInfo);
            Logger.Write("clusterInfo.NumberOfInstances : " + clusterInfo.NumberOfInstances);
        ...
        public void postPrimaryEvent(ISpaceProxy spaceProxy, SpaceMode spaceMode)
        {
            ...
            Logger.Write(">>>>> post primary -> postPrimaryEvent Version: " + Version + " Partions: " + totalSpaces.ToString()
            ...
            Logger.Write(string.Format("lastGSOrderID: {0} lastGSFillID : {1}", lastGSOrderID, lastGSFillID));
        ...
        class OP
        {
            ...
            {
                Logger.Write(" startPU Sleeping ");
                Thread.Sleep(60*1000);
                Logger.Write(" startPU Running ");
                ...
                Logger.Write(string.Format("*** PartitionID:{0} Connected to space.", partionId));
                ...
                Logger.Write(string.Format("PartitionID:{0} Total Time taken  : {1}", partionId, ((orderProcesserTotalEndime - orderProcesserTotalStartTime) / TimeSpan.TicksPerMillisecond)));
                ...
                Logger.Write(string.Format("Test Config: PartitionID:{0}, Orders: {1}, Fills: {2}, OrderThreads: {3},  FillThreads: {4}",
                ...
                Logger.Write(string.Format("Wrote Orders: {0} in {1} ms, average {2} ms, {3} orders in sec, {4} start time, {5} end time, PartitionID:{6}  ",
                ...
                Logger.Write(string.Format(">>>>>> Updated code : Wrote Orders: {0} in {1} ms, average {2} ms, {3} orders in sec, {4} start time, {5} end time, {6} minTicks, {7} maxTicks, PartitionID:{8} ",
                ...
                {
                    Logger.Write(string.Format("Order Stat partionId:{0} - {1} samples taken, each one averaging {2} orders records.",
                                  partionId, orderstatRecord.N, orderSampleSize));
                    Logger.Write(string.Format("Order Stat partionId:{0} - Aggregate Latency: {1}.",
                ...
                Logger.Write(string.Format("Wrote Fills: {0} in {1} ms, average {2} ms, {3} fills in sec, PartitionID:{4}, {5} start time, {6} end time", fillCnt, fillTimems, avgFill, fillMsgPerSec,
                ...
                Logger.Write(string.Format(">>>>>> Updated code : Wrote Fills: {0} in {1} ms, average {2} ms, {3} fills in sec , {4} minTicks, {5} maxTicks, PartitionID:{6}, {7} start time, {8} end time ",
                ...
                {
                    Logger.Write(string.Format("Fill Stat partionId:{0} - {1} samples taken, each one averaging {2} fills records.",
                                  partionId, fillstatRecord.N, fillSampleSize));
                    Logger.Write(string.Format("Fill Stat partionId:{0} - Aggregate Latency: {1}.",
                ...
                Logger.Write("Order Processor finished successfully!");
